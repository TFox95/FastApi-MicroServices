...
async def checkAuthorization(request: Request, Authorization=Cookie(None)) -> str:
    ...
    try:
        ...
        raise HTTPException(
            detail={"error": f"Token invalid"}, status_code=status.HTTP_401_UNAUTHORIZED)
...
async def getCurrentUser(token=Depends(checkAuthorization), db: Session = Depends(get_db)) -> UserModel:
    try:
        ...
        raise HTTPException(detail={"error": "Internal Error"},
...
async def getAllUsers(request: Request, db: Session = Depends(get_db)):
    ...
    if not grabUsers:
        return JSONResponse({"error": "Something went wrong, please Try again later"}, status.HTTP_400_BAD_REQUEST)
    return grabUsers
